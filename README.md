...
让 Codex 从本仓库安装 `image-watermark/` 目录，或手动执行：

```bash
git clone https://github.com/yxzwork89/image-watermark.git
cp -R image-watermark/image-watermark ~/.codex/skills/
```

安装后，在新的 Codex 任务中使用 `$image-watermark`。
...
```bash
python3 image-watermark/scripts/apply_watermark.py \
